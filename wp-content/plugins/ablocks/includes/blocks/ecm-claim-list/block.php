...
namespace ABlocks\Blocks\EcmClaimList;
...
class Block extends BlockBaseAbstract {
	protected $block_name = 'ecm-claim-list';

	public function build_css( $attributes ) {
		$css_generator = new CssGenerator( $attributes, $this->block_name );

		$css_generator->add_class_styles(
			'{{WRAPPER}} .ecm-shortcode--claim__list-item',
		...
		$css_generator->add_class_styles(
			'{{WRAPPER}} .ecm-shortcode--claim__list',
		...
		$css_generator->add_class_styles(
			'{{WRAPPER}} .ecm-shortcode--claim__list-title',
		...
		$css_generator->add_class_styles(
			'{{WRAPPER}} .ecm-shortcode--claim__list-date',
		...
		$css_generator->add_class_styles(
			'{{WRAPPER}} ul.ecm-shortcode--claim__list',
		...
		$css_generator->add_class_styles(
			'{{WRAPPER}} .ecm-shortcode--claim__list-item:hover',
			$this->getListHoverCSS( $attributes ),
			$this->getListHoverCSS( $attributes, 'Tablet' ),
			$this->getListHoverCSS( $attributes, 'Mobile' ),
		);
		$css_generator->add_class_styles(
			'{{WRAPPER}} .ecm-shortcode--claim__not-found',
			$this->getNotFoundTextStyle( $attributes ),
			$this->getNotFoundTextStyle( $attributes, 'Tablet' ),
			$this->getNotFoundTextStyle( $attributes, 'Mobile' ),
		);
	...
	public function render_block_content( $attributes, $content, $block_instance ) {
		...
		$shortcode = '[ecm_claim_list ' . Helper::attr_shortcode( $attr_array ) . ']';
	...
	public function getNotFoundTextStyle( $attributes, $device = '' ) {
		$css = [];
		 if ( ! empty( $attributes['notFoundAlignment'] ) && ! empty( $attributes['notFoundAlignment']['value' . $device] ) ) {
			$css['text-align'] = $attributes['notFoundAlignment']['value' . $device];
		}

		if ( ! empty( $attributes['notFoundPadding'] ) ) {
			$css = array_merge(
				$css,
				Dimensions::get_css($attributes['notFoundPadding'], 'padding', $device)
			);
		}
		if ( ! empty( $attributes['notFoundColor'] ) && $attributes['notFoundColor'] !== '' ) {
			$css['color'] = $attributes['notFoundColor'];
		}

		$typographyValueGlobal = ! empty( $attributes['notFoundTypographyGlobal'] ) ? $attributes['notFoundTypographyGlobal'] : '';
		$typography_css = ! empty( $attributes['notFoundTypography'] ) ? Typography::get_css( $attributes['notFoundTypography'], '', $device, $typographyValueGlobal ) : array();
		return array_merge(
			$css,
			$typography_css
		);
